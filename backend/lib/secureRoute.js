...
const User = require('../models/user.model') // * and the user model, to look up the user trying to use the token
...
async function secureRoute(req, res, next) {
  try {
    ...
    const payload = await jwt.verify(token, process.env.SECRET) // * attempt to decode that token
